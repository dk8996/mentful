handleMouseMove = function(element){
 ...
 socket.emit('sendEvent', { event: 'click', x: xy[0], y: xy[1]});

socket.on('update', function (data) {
console.log('Someone sent a update event');
var xy = scale(data.x,data.y,270,180,720,480,5,5);
var newdiv = $("<div/>").css('background','url("dot.png")').css('position','absolute').css('width','10px').css('height','10px').css('top',xy[1]+'px').css('left',xy[0]+'px');
$( "#image" ).append(newdiv);
...
var newdiv = $("<div/>").css('background','url("dot.png")').css('position','absolute').css('width','50px').css('height','50px').css('top',xy[1]+'px').css('left',xy[0]+'px');
$( "#image" ).append(newdiv);
});
